namespace Tyuiu.SenachevAV.Sprint7.Project.V7.Lib
{
    public class DataService
    {
        public string[,] GetMatrix(string path)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                {
                    if (c < columns) // ПОЧЕМУ ОН ЖАЛУЕТСЯ НА СТОЛБЦЫ
                    {
                        matrix[i, c] = strArr[c];
                    }
                    else
                    {
                        ...
                        Console.WriteLine($"Строка {i} имеет больше элементов, чем допустимо в matrix.");
                    }
                }
            }
            return matrix;
        ...
        {
            string[] strLines = File.ReadAllLines(path); //Корень всех бед
            bool res = false;
